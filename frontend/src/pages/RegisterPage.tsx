...
import { TrendingUp, Lock, Mail, Globe, AlertCircle, ArrowRight, CheckCircle2 } from 'lucide-react';
...
export const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await register({ email, password, preferred_market: preferredMarket });
      navigate('/dashboard');
    } catch (err: any) {
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                <select
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-3 text-xs font-medium text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-slate-800 dark:bg-slate-900/80 dark:text-white"
                >
                  <option value="US">United States (USD - NYSE/NASDAQ)</option>
                  <option value="IN">India (INR - NSE/BSE)</option>
                  <option value="UK">United Kingdom (GBP - LSE)</option>
                  <option value="JP">Japan (JPY - TSE)</option>
                  <option value="DE">Germany (EUR - XETRA)</option>
                  <option value="HK">Hong Kong (HKD - HKEX)</option>
                  <option value="CA">Canada (CAD - TSX)</option>
                  <option value="AU">Australia (AUD - ASX)</option>
                </select>
              </div>
            </div>
        ...
        <div className="flex h-full flex-col justify-between p-12 text-white">
          <div className="space-y-4 max-w-lg">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-400 border border-emerald-500/20">
              <CheckCircle2 className="h-3.5 w-3.5" /> Get Started in Seconds
            </span>
            <h1 className="text-3xl font-extrabold leading-tight">
              Unified Stock Portfolios for Global Investors
            </h1>
            <p className="text-sm text-slate-400 leading-relaxed">
              Create a custom watchlist, stream tick-by-tick quotes, configure multi-condition price alerts, and analyze market trends across 8 global exchanges.
            </p>
          </div>
